...
public enum Lsp {
	...
	public protocol Handler: Actor {
		...
		/// # Params
		/// - request: ``RequestMessage`` an lsp request message sent by the client to
		/// the server
		/// # Returns:
		/// ``ResponseMessage`` an lsp response message sent by the server to the
		/// client
		func handle(request: RequestMessage) async -> ResponseMessage
		/// Handles a notification without returning a response message.
		/// # Params
		/// - notification: ``NotificationMessage`` an lsp notification sent by the
		/// client to the server
		func handle(notification: NotificationMessage) async
	}

	/// A Proxy handler that does not process lsp messages. It rather redirect the
	/// messages through a tcp connection
	...
	>: Handler {
		...
		/// a dictionary that holds pending requests, in order to map responses back
		/// to the requests and return from the async ``handle()`` calls
		private var pendingRequests:
			[Id: CheckedContinuation<ResponseMessage, Never>] = [:]

		/// Creates a ``ProxyHanlder``
		/// # Usage
		/// In order to properly use the proxy handler, it needs to be started before
		/// the lsp server that uses it.
		/// To start the server call the function ``run()``
		/// # Params
		/// - client: ``Socket.TcpClient`` the tcp client to forward messages to and
		/// from
	...
	/// Transport protocol defines the interface for reading and writing data
	/// between an lsp server and lsp client
	...
	/// The run method is blocking and will be using the transport to read incoming
	/// messages from the transport input
	/// and write messages into the transport output
	public actor Server<H: Handler, T: Transport, L: Utils.Logger> {
		/// Rpc encoder and decoder, responsible for serializing rpc messages from and
		...
		/// Runs the server.
		/// The call blocks and only ends when the lsp client ends the session or an
		/// error occurs
		/// # Throws
		...
		public func run() async throws {
			...
			while true {
				...
				let decodedMessage = coder.decode(data: data)
